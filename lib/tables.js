...
(function (global) {
  ...
  function save(t, cfg) {
    return global.WFStore.set(t, "board", KEY, {
   ...
   *
   * THE SHOP FLOOR MUST NOT CARE HOW A JOB LEAVES. This used to read
   ...
   * reports a problem.
   *
   * It is WFPhase's question now, not this file's, and there is deliberately no
   * local fallback: keeping a second copy is how the two drifted the first time
   * -- phase.js changed what "finished" means and the shop floor went on
   * believing the old answer for weeks. A hard dependency fails loudly on a
   * surface that loads tables.js without phase.js, which is a load-order
   * mistake worth finding immediately rather than a wrong answer worth
   * shipping. Every surface that loads this file loads phase.js first.
   */
  function isFinished(work) {
    return global.WFPhase.isFinished(work);
  }
