...
(function (global) {
  ...
  function stationState(boardCfg, cards, station) {
     ...
     */
    /* RUNNING FIRST, THEN PAUSED. This ordering is the whole switch feature.
     *
     * Pausing job A and starting job B leaves BOTH carrying this station's
     * tableId -- A is parked here at the front of the queue, B is on the clock.
     * Taking the first match meant the station often kept showing A, so
     * switching looked like it silently failed and the same tap was tried
     * again. The station shows whatever is on the clock; a paused job only
     * takes the slot when nothing is running. */
    var mineHere = inPhase.filter(function (c) {
      var w = activeWork(c);
      return w && !isFinished(w) && w.tableId === station.id;
    });
