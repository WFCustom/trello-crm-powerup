...
(function (global) {
  ...
  var DEFAULT_MODEL = {
    categories: [
      ...
      { name: "Shop", pct: 35,
        // CNC is billed as Shop work, but it is not a step every job takes --
        // see OPTIONAL_PHASES. Some jobs are only CNC; on others the frame is
        // built before the cut-out goes in.
        phases: ["CAD", "Print CAD", "CNC Table", "CNC Table/cap rail", "Assemble"] },
  ...
  /**
   * Phases a job may skip entirely. CNC is the case that prompted this: often
   * it is a component of a rail rather than a stage of one, sometimes it is the
   * whole job, and the frame is frequently built before the cut-out exists. So
   * it should never be forced into the sequence -- a job goes through it only
   * when there is actually CNC work.
   */
  var OPTIONAL_PHASES = ["CNC Table", "CNC Table/cap rail"];
