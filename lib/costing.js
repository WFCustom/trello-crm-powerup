...
(function (global) {
    ...
    categories: [
      { name: "Sales & Office", pct: 30,
        phases: ["Intake", "Portal - CRM", "Make Job Packet", "Portal - Final Measure",
                 "Final Approval", "Billing", "Outstanding Invoices"] },
      { name: "Shop", pct: 35,
        // CNC is billed as Shop work. It is no longer a stage of its own --
        // cutting happens inside Assemble CNC -- so there is nothing left to
        // mark optional here.
        phases: ["CAD", "Print CAD", "Assemble Legacy", "Assemble CAP", "Assemble CNC"] },
      { name: "Sand & Powder", pct: 25,
        phases: ["Sandblast / Powder Coat"] },
      // The four Install columns report as one "Install" phase (see the phase
      // field in config.js), so one entry covers all of them. The old raw names
      // are kept so historical phaseLog entries still bucket correctly.
      { name: "Install", pct: 10,
        phases: ["Install", "Install (Tuesday)", "Install North", "Install Central",
                 "Install South", "Next week Install"] }
    ],
  ...
  /* Timeframes are measured from when a job entered CAD -- the point it became
     real work rather than a lead. */
  var TIMEFRAMES = [
  ...
  /**
   * When the job started, for timeframe filtering.
   *
   * Exact source is the CAD entry in phaseLog. Cards that predate the Power-Up
   * have no phaseLog, so we fall back to the earliest entry we do have, then to
   * the card's last activity -- a rough proxy, flagged as approximate so a
   * report never implies more precision than it has.
   */
  function jobStartDate(card) {
    var log = (card && card.phaseLog) || [];
    var cad = null, earliest = null;
    log.forEach(function (e) {
      var when = e.approvedAt || e.completedAt;
      if (!when) return;
      var name = String(e.listName || e.name || "");
      if (/^\s*CAD\b/i.test(name) && (!cad || when < cad)) cad = when;
      if (!earliest || when < earliest) earliest = when;
    });
    if (cad) return { at: cad, exact: true };
    if (earliest) return { at: earliest, exact: false };
    if (card && card.dateLastActivity) return { at: card.dateLastActivity, exact: false };
    return { at: null, exact: false };
  }

  function withinTimeframe(card, key) {
    var tf = TIMEFRAMES.filter(function (x) { return x.key === key; })[0];
    if (!tf || tf.days == null) return true;
    var start = jobStartDate(card);
    if (!start.at) return false;
    var age = (Date.now() - new Date(start.at).getTime()) / 86400000;
    return age <= tf.days;
  }

  /**
   * Phases a job may skip entirely, so their absence is never read as a job
   * running late or a step being missed.
   *
   * CNC was the case that prompted this, back when cutting had its own column.
   * It no longer does -- cutting and building both happen in Assemble CNC -- and
   * the three assemble columns are already mutually exclusive by job type, which
   * the route in lib/jobtype.js expresses directly. Sandblast / Powder Coat is
   * the one genuine skip left: CAP railing is aluminium and never gets a finish
   * coat, and some CNC-only work ships raw.
   */
  var OPTIONAL_PHASES = ["Sandblast / Powder Coat"];

  function optionalPhases() {
    var c = global.WF_CONFIG && global.WF_CONFIG.optionalPhases;
    return Array.isArray(c) && c.length ? c : OPTIONAL_PHASES;
  }

  function isOptionalPhase(name) {
    return optionalPhases().indexOf(name) !== -1;
  }
  ...
  function buildJobRows(cards, boardCfg, rates) {
    ...
    return (cards || []).map(function (card) {
      ...
      /* Actuals typed against the job. Kept as named buckets rather than one
         "cost" figure so the pie can break them out and so entering materials
         later doesn't silently overwrite consumables. econ.cost is the older
         single-figure field and is still honoured. */
      var a = econ.actuals || {};
      var materials = Number(a.materials) || 0;
      var consumables = Number(a.consumables) || 0;
      var otherCost = Number(a.other) || 0;
      var enteredCost = materials + consumables + otherCost + (Number(econ.cost) || 0);
  ...
  function rollup(rows) {
    var t = {
      ...
      materials: 0, consumables: 0, otherCost: 0,
    ...
    rows.forEach(function (r) {
      ...
      t.consumables += r.consumables;
      t.otherCost += r.otherCost + r.legacyCost;
  ...
  global.WFCosting = {
    ...
    optionalPhases: optionalPhases, isOptionalPhase: isOptionalPhase,
    jobStartDate: jobStartDate, withinTimeframe: withinTimeframe,
