...
(function (global) {
  ...
  function saveTraining(t, rows) {
    ...
    if (json.length > 3800) {
      return Promise.reject(new Error(
        "That's too much for one training record to hold (" + json.length +
        " of 4096 characters). Remove a few finished rows and save again."));
    }
    return t.set("board", "shared", TRAINING_KEY, rows || []);
  }
