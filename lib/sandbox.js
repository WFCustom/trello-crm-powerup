...
(function (global) {
  ...
  function wrap(realT) {
    ...
    /* THE SHIM HAS TO SPEAK EVERY SHAPE THE REAL API DOES.
     *
     * Trello's t.get and t.set both take more than one form, and the sandbox
     * originally understood only the four-argument one. That was invisible
     * until something used another form: WFStore measures a scope with
     * `t.get(scope, visibility)` and no key, and writes several keys at once
     * with `t.set(scope, visibility, patchObject)`. Under the old shim the
     * patch object would have been stored AS A KEY -- `slot[<object>]` -- so
     * test mode would have silently recorded garbage and shown the operator a
     * rehearsal that bore no relation to what the live path does.
     *
     * A sandbox whose fidelity depends on which overload you happened to pick
     * is worse than no sandbox, because people trust it. */

    shim.get = function (scope, vis, key, dflt) {
      // No key: the caller wants the whole blob at this scope. Merge the
      // overlay over the real one so a measurement sees this session's writes.
      if (key === undefined) {
        return Promise.resolve(realT.get(scope, vis)).then(function (real) {
          var out = {};
          if (real && typeof real === "object") {
            Object.keys(real).forEach(function (k) { out[k] = real[k]; });
          }
          if (on && overlay[scope]) {
            Object.keys(overlay[scope]).forEach(function (k) {
              if (overlay[scope][k].removed) delete out[k];
              else out[k] = overlay[scope][k].value;
            });
          }
          return out;
    ...
    shim.set = function (scope, vis, key, value) {
      ...
      // The object form: t.set(scope, vis, {k: v, ...}). Trello's own docs
      // recommend it over several separate calls, because separate calls
      // read-modify-write one shared blob and the later one discards the
      // earlier. Recorded key by key so the log still reads as sentences.
      if (key && typeof key === "object" && value === undefined) {
        Object.keys(key).forEach(function (k) {
          slot(scope)[k] = { value: key[k] };
          record("data", describeSet(scope, k, key[k]),
            { scope: scope, key: k, value: key[k] });
        });
    ...
    shim.remove = function (scope, vis, key) {
      ...
      // Also documented as accepting an array of keys.
      [].concat(key).forEach(function (k) {
        slot(scope)[k] = { removed: true };
        record("data", "Cleared " + k + " on " + shortId(scope),
          { scope: scope, key: k, removed: true });
      });
      return Promise.resolve();
    };
