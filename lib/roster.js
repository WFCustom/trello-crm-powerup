...
(function (global) {
  ...
  function defaultsFromConfig() {
    ...
    return {
      ...
      // Hand-entered hourly rates, username -> dollars. QuickBooks rates win
      // over these when a sync exists; until then this is the only source, and
      // without it all labour costs zero and every margin reads 100%.
      rates: (cfg.hourlyRates && typeof cfg.hourlyRates === "object")
        ? JSON.parse(JSON.stringify(cfg.hourlyRates)) : {},
  ...
  async function getRoster(t) {
    ...
    return {
      ...
      rates: (stored.rates && typeof stored.rates === "object")
        ? JSON.parse(JSON.stringify(stored.rates)) : base.rates,
      phaseSpecialists:
  ...
  async function saveRoster(t, roster) {
    await global.WFStore.set(t, "board", KEY, roster, { label: "the roster" });
    return roster;
  }
  ...
  async function setRate(t, username, dollarsPerHour) {
    const roster = await getRoster(t);
    const n = Number(dollarsPerHour);
    if (!username) return roster;
    if (!isFinite(n) || n <= 0) delete roster.rates[username];
    else roster.rates[username] = n;
    return saveRoster(t, roster);
  }
