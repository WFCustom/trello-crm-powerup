...
(function (global) {
   ...
   */
  function getStationChecklist(t, stationId, phaseName) {
    return getStationChecklists(t).then(function (all) {
      var own = all[stationId];
      // A saved empty list is a deliberate choice and is respected.
      if (Array.isArray(own)) return own.map(normItem);
      // Then whatever a manager saved for the phase in the Roster tab, so the
      // two editors don't disagree about the same work.
      return getTemplates(t).then(function (byPhase) {
        var saved = byPhase[phaseName];
        if (Array.isArray(saved)) return saved.map(normItem);
        return defaultStationItems(phaseName) || defaultTemplate(phaseName).map(normItem);
      });
    });
  }
  ...
  function signOffAndPass(t, cardMeta, opts) {
    ...
    // silent: the job is not waiting for anybody. approveAndAdvance follows
    // immediately and writes the comment that is actually true.
    return put(t, cardMeta, rec)
      .then(function () { return WFPhase.complete(t, cardMeta, { silent: true }); })
      .then(function () { return WFPhase.approveAndAdvance(t, cardMeta, person(signer)); })
