...
(function (global) {
  ...
  /* The size constants that used to live here are gone: they were a per-record
     ceiling, and the real limit is per scope. lib/store.js owns them now. */
  ...
  /**
   * THIS USED TO MEASURE THE WRONG THING, AND THE FILE HEADER STILL SAYS SO.
   *
   * The check here compared ONE record against 3,800 characters. Trello's limit
   * is per scope/visibility pair -- every EOS record shares one 4,096-character
   * board budget with the roster, the permissions, the station setup and the
   * checklist library. So this passed at 3,700 while the board sat at 15,000,
   * and the write failed anyway with a different, worse message.
   *
   * The note further up this file about splitting five keys to dodge the cap is
   * wrong for the same reason. Splitting bought nothing; it is left in place
   * only because merging them now would be churn for no gain.
   */
  function saveRecord(t, name, value) {
    var key = KEYS[name];
    if (!key) return Promise.reject(new Error("No EOS record called " + name + "."));
    return global.WFStore.set(t, "board", key, value, { label: "the " + name });
  }

  /* recordSize() was removed rather than re-pointed. It measured one record
     against a per-record ceiling that does not exist; the honest answer is
     WFStore.usage(t, "board"), which is what the EOS tab now calls directly.
     A wrapper that only forwards is one more place for the question to drift. */
