...
(function (global) {
  ...
  /**
   * Everything needed to draw a card without opening Trello, in ONE request.
   *
   * Trello lets nested resources ride along on /cards/{id}, so the cover, the
   * labels, the members, the checklists, the attachments and the comment
   * thread all arrive together. Fetching them separately was six round trips
   * per card, which is why the old dashboard sent you to Trello instead of
   * showing you anything.
   *
   * `attachments=true` is what makes a cover resolvable: Trello's `cover`
   * object only carries an id, and the actual image URL lives on the
   * attachment record it points at.
   */
  async function getCardDetail(t, cardId, opts) {
    const fresh = opts && opts.fresh;
    const cacheKey = "detail:" + cardId;
    if (!fresh) {
      const cached = cacheGet(cacheKey);
      if (cached) return cached;
    }
    const card = await request(t, "/cards/" + cardId, {
      fields: "name,desc,due,dueComplete,idList,idBoard,idMembers,shortUrl,pos,labels,cover,closed,dateLastActivity",
      attachments: "true",
      attachment_fields: "name,url,mimeType,isUpload,date,previews",
      checklists: "all",
      checklist_fields: "name,pos",
      members: "true",
      member_fields: "fullName,username,initials,avatarUrl",
      actions: "commentCard",
      actions_limit: 20,
      board: "true",
      board_fields: "name",
  ...
  /** Drop every cached view of one card, so the next read sees the write. */
  function invalidateCard(cardId) {
    ["detail:", "attachments:", "history:", "customfields:"].forEach((p) => {
      cache.delete(p + cardId);
    });
  }

  /** PUT any of name / desc / due / dueComplete / idMembers onto a card. */
  async function updateCard(t, cardId, params) {
    const out = await write(t, "PUT", "/cards/" + cardId, params);
    invalidateCard(cardId);
    return out;
  }

  /** Tick or untick one checklist item. `state` is "complete" or "incomplete". */
  async function setCheckItem(t, cardId, checkItemId, state) {
    const out = await write(t, "PUT",
      "/cards/" + cardId + "/checkItem/" + checkItemId, { state: state });
    invalidateCard(cardId);
    return out;
  }

  /** Set one custom field's value on a card. `body` is Trello's value shape. */
  async function setCustomField(t, cardId, fieldId, body) {
    const url = await buildUrl(t, "/cards/" + cardId + "/customField/" + fieldId + "/item", {});
    const res = await fetch(url, {
      method: "PUT",
    ...
    if (!res.ok) {
      const text = await res.text().catch(() => "");
      throw new Error("Trello API " + res.status + " on custom field: " + text.slice(0, 200));
    }
    invalidateCard(cardId);
  ...
  async function getBoardCardsFull(t, boardId, opts) {
    ...
    const cards = await request(t, "/boards/" + boardId + "/cards", {
      // due/dueComplete power "is this late"; pos is the board's own manual
      // order, which the station queues read so dragging a card up in Trello
      // reorders the shop floor with no second place to keep the priority.
      // idBoard is not decoration. Cards from here get passed straight to
      // WFPhase as cardMeta, and every stage lookup is keyed on (board, list):
      // without it getStageForList and getNextStage both answer null, so a
      // completed job never moves and the job-type router bails out before it
      // starts. It was missing, and the failure was silent in both places.
      fields: "name,idBoard,idList,dateLastActivity,shortUrl,desc,closed,due,dueComplete,pos,idMembers",
      filter: filter,
      pluginData: "true"
    });
    const parsed = cards.map((c) => {
      ...
      return {
        id: c.id,
        name: c.name,
        // The comment above about idBoard applies HERE as much as to the
        // `fields` string. Asking Trello for it and then dropping it on the
        // floor in this projection is the same bug twice: the field arrived and
        // never reached a caller. Every card in this list is handed to WFPhase
        // as cardMeta, and stage lookups key on (board, list).
        idBoard: c.idBoard,
        idList: c.idList,
        ...
        // The Records tab reads the QC archive off the board in one pass; without
        // this it would be a REST call per card to answer "what failed last month".
        qcRecord: shared.qcRecord || null
      };
    });
