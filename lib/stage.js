...
(function (global) {
  ...
  // Find a stage by its phase NAME rather than its list id. Several lists share
  // a name on purpose -- the four Install columns are one phase of work -- so
  // when they do, prefer the one flagged isPrimaryTarget, exactly as
  // getNextStage does above. Without this rule a name lookup silently picks
  // whichever Install column happens to be listed first, which is "Next week".
  function getStageByName(boardId, name) {
    const board = getBoardConfig(boardId);
    if (!board || !name) return null;
    const matches = board.stages.filter((s) => s.name === name);
    if (!matches.length) return null;
    return matches.find((s) => s.isPrimaryTarget) || matches[0];
  }
