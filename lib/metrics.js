...
(function (global) {
  ...
  // Management overhead: a flat % of job VALUE, added to a card's cost,
  // whenever an owner/manager username (config.js managementOverhead.usernames)
  // shows up as claimedBy anywhere in that card's phase log. Never a named
  // rate -- just one blended, unattributed line, so Team Performance and
  // per-job margin stay consistent with the card badge / Job Economics popup
  // without ever surfacing an individual's actual pay.
  function managementOverheadAmount(card, value) {
    const cfg = (global.WF_CONFIG && global.WF_CONFIG.managementOverhead) || null;
    if (!cfg || !value) return 0;
    const mgmtUsernames = cfg.usernames || [];
    const log = card.phaseLog || [];
    const touched = log.some((e) => e.claimedBy && mgmtUsernames.indexOf(e.claimedBy.username) !== -1);
    return touched ? Math.round(value * ((cfg.percentOfJobValue || 0) / 100)) : 0;
  }
  ...
  function buildProjectRollup(cards, boardCfg) {
    return cards.map((card) => {
      ...
      const baseCost = econ && econ.cost != null ? Number(econ.cost) : null;
      const managementOverhead = managementOverheadAmount(card, value);
      const cost = baseCost != null || managementOverhead
        ? (baseCost || 0) + managementOverhead
        : null;
      ...
      return {
        ...
        managementOverhead: managementOverhead || null, // blended $, never a named rate
        totalMinutes,
        productionDays,
  ...
  // Owner/managers are deliberately never added to hourlyRates/qbEmployeeMap,
  // so their rows always fall into the "no rate" case here -- hours and
  // revenue share only, no wage ever surfaces in this view.
  function buildPersonRollup(cards, boardId, sinceDate, liveRates) {
    ...
    cards.forEach((card) => {
      ...
      const cardValue = (card.economics && card.economics.value != null) ? Number(card.economics.value) : null;
      const cardBaseCost = (card.economics && card.economics.cost != null) ? Number(card.economics.cost) : null;
      const cardOverhead = managementOverheadAmount(card, cardValue);
      const cardCost = cardBaseCost != null || cardOverhead ? (cardBaseCost || 0) + cardOverhead : null;
      const cardMargin = (cardValue != null && cardCost != null) ? cardValue - cardCost : null;
