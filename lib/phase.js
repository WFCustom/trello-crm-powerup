...
(function (global) {
  ...
  async function undoComplete(t, cardMeta) {
    const work = await getActivePhaseWork(t, cardMeta);
    // Either flag means finished. Gating on pendingApproval alone made this a
    // SILENT no-op the moment a path stopped writing that flag -- the button
    // stayed, did nothing, and reported nothing. complete() sets both; the
    // second clause goes when pendingApproval does.
    if (!work || !(work.completedAt || work.pendingApproval)) return work;
