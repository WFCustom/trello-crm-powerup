...
(function (global) {
  ...
  /**
   * May this person see money?
   *
   * Accepts either the ops context -- which carries `can` and therefore the
   * board's own permission settings -- or a plain role string. The context path
   * is the real one; the string path is the fallback for callers that predate
   * permissions, and for the tests that describe the rule in role terms.
   *
   * Note the asymmetry with the old rule: "office" no longer implies money.
   * Under the three tiers, costing is a grant somebody was given rather than a
   * side effect of not being on the shop floor.
   */
  function canSeeMoney(who) {
    if (who && typeof who.can === "function") return who.can("see.costing");
    return who === "manager" || who === "office";
  }
   ...
   */
  function visibleFields(fields, who) {
    if (canSeeMoney(who)) return (fields || []).slice();
   ...
   */
  function editRights(who) {
    if (who && typeof who.can === "function") {
      return {
        title: who.can("edit.card"),
        description: who.can("edit.card"),
        due: who.can("edit.due"),
        checkItems: true,
        comment: true,
        money: who.can("see.costing")
      };
    }
    var manage = who === "manager" || who === "office";
    return {
      title: manage,
      description: manage,
      due: manage,
